...
import { FormatCreditCard, FormatZip, IsNumber } from '@/components/global/functions/Functions';
	...
	const handleSubmit = async (e) => {
		...
		try {
			if (monthly === 'ach') {
				...
				const response = await fetch(`${process.env.API_URL}/physicians/office/setup/edit/bank/ach`, {
					method: 'PUT',
					headers: {
				...
				const response = await fetch(`${process.env.API_URL}/physicians/office/setup/edit/bank/cc`, {
					method: 'PUT',
					headers: {
						'Content-Type': 'application/json',
					},
					...
					{monthly === 'cc' && (
						<form onSubmit={handleSubmit}>
							...
							<div className='mb-2 flex flex-row items-center'>
								<div className='w-4/5'>
									<Input type='tel' required={true} value={ccnum} funcCall={handleCCNum} />
								</div>
								<div className='w-1/5'>
							...
							<div className='mt-5 flex justify-center'>
								<Button type='submit' disabled={!ccnum || !ccexpmo || !ccexpyr || !cvv || !cczip}>
									Make Payment
								</Button>
							</div>
						</form>
