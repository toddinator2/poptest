...
export default function Procenter() {
	...
	const handleSubmit = async (e) => {
		e.preventDefault();
		try {
			const response = await fetch(`${process.env.API_URL}/physicians/office/setup/edit/policies/procenter`, {
				method: 'PUT',
				headers: {
	...
	return (
		<div className='w-full pb-5 lg:w-5/6 2xl:w-3/4 lg:mx-auto flex flex-col xl:flex-row xl:justify-center gap-3 2xl:gap-6'>
			<div className='w-5/6 md:w-2/3 xl:w-1/3 mx-auto mb-3 xl:mb-0 border-4 border-drkred rounded-2xl order-2 xl:order-1'>
				<div className='w-full py-2 font-semibold text-center text-xl border-b-4 border-b-drkred'>PRO-CENTER AGREEMENT</div>
			...
			<div className='w-5/6 md:w-2/3 xl:w-1/3 mx-auto mb-3 xl:mb-0 border-4 border-drkblu rounded-2xl order-1 xl:order-2'>
				...
				<div className='w-5/6 mx-auto py-3 flex flex-col'>
					<div className='mb-3 text-lg font-semibold text-center'>SN3X PRO-CENTER AGREEMENT</div>
					<div>Please read and check the &quot;I Accept&quot; checkbox and type in your initials to accept the Pro-Center Agreement.</div>
				</div>
			</div>
