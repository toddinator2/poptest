		...
		<div>
			{!patient.hide && (
				<div className='docs-appointment-task' ref={setDragElm}>
					{patient.photo ? (
						<Image className='mx-2 rounded-full cursor-pointer' src={patient.photo} width={50} height={50} alt='Patient Photo' />
					) : (
						<div className='mx-2 text-4xl font-bold text-txtblu cursor-pointer'>{hdrInits}</div>
					)}
...
export const Header = () => {
	...
	return (
		<div className='py-2 mbsc-col-sm-12 docs-appointment-cont' ref={setDropElm}>
			<Dropcontainer onItemDrop={handleItemDrop} onItemDragEnter={handleItemDragEnter} onItemDragLeave={handleItemDragLeave} element={dropCont}>
				<div className='text-lg font-semibold mbsc-form-group-title'>Today&apos;s Appointments</div>
				<div className='flex flex-row items-center'>
					<div className='w-full ps-5 mb-3'>
						{apptsToday.length !== 0 ? (
							<>
								{apptsToday.map((appt) => (
							...
							<div className='text-base font-semibold text-drkred'>No Appointments Today</div>
						)}
					</div>
				</div>
			</Dropcontainer>
