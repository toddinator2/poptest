...
export default function SpnLogin() {
	...
	useEffect(() => {
		...
		if (!setAuth) {
			const lsData = GetAuthData('sponsor');
			...
			if (result?.error) {
				...
				if (usrStatus === 200) {
					if (!user.resetcreds && user.permission === 'sponsor') {
						...
						SaveAuthData('sponsor', saveAuthData);
						router.push('/sponsors/sphere');
					} else {
						if (user.permission !== 'sponsor') {
							router.push('/sponsors/login');
						} else {
							router.push(`/sponsors/login/resetcreds/${user.resetcode}`);
						}
					}
				}
			}
					...
					<div className='row d-flex justify-content-center'>
						<div className='col-8 col-md-4 col-xl-3 '>
							...
							<div className='mt-4 d-flex justify-content-center'>
								<Button type='submit' border='8000FF' disabled={!uname || !pword}>
									Login
								</Button>
							</div>
						</div>
